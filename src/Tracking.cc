...
* 
* Modification: PO-SLAM
* Version: 1.0
* Modified: 08/29/2023
* Author: Yaolin Tien
* E-mail: tianyaolin21@mails.ucas.ac.cn
*/
...
//string mSeq = "rgbd_dataset_freiburg3_long_office_household";
...
Tracking::Tracking( System *pSys, ORBVocabulary *pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Map *pMap, KeyFrameDatabase *pKFDB, 
                    const string &strSettingPath, const int sensor, const string &flag, const string &seqName) : 
                    ...
                    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
{
    ...
    // MONO_Offline: read groundtruth --------------------------------------------------
    // notice: Only the camera pose of the initial frame is used to determine the ground plane normal.
    if (sensor == System::MONOCULAR && mbReadedGroundtruth == false)
    {
        ifstream infile("./dataset/SePT/" + mSeq + "/" + "groundtruth.txt", ios::in);
    ...
    // STEREO_Offline: read groundtruth --------------------------------------------------
    if (sensor == System::STEREO && mbReadedGroundtruth == false)
    {
        // for no pose initialization
        // ifstream infile("./dataset/SePT/" + mSeq + "/mav0/state_groundtruth_estimate0/data.csv", ios::in);
        // for ground-truth pose initialization
        ifstream infile("./dataset/SePT/" + mSeq + "/mav0/state_groundtruth_estimate0/data.txt", ios::in);
        if (!infile.is_open())
        {
            cout << "SePT state_groundtruth file open fail" << endl;
            exit(233);
        }
        else
        {
            std::cout << "Read state_groundtruth...done" << std::endl;
            mbReadedGroundtruth = true;
        }
        ...
        // string s0;               // 跳过文件头首行 2023.11.18
        // getline(infile, s0);     // 跳过文件头首行 2023.11.18
...
cv::Mat Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, const bool bSemanticOnline)
{
    frame_id_tracking++;

    cv::Mat rawImage = imRectLeft.clone();
    ...
    mCurrentFrame = Frame(rawImage, mImGray, imGrayRight, timestamp, mpORBextractorLeft, mpORBextractorRight, 
                            line_lbd_ptr,       // added: line extractor.
                            mpORBVocabulary, mK, mDistCoef, mbf, mThDepth, 
                            mImGray, rawImage); //added: gray_imu & rbg_imu
    ...
    {
        // offline object box.
        std::stringstream str_ts;
        str_ts << fixed << setprecision(0) << timestamp*1e9;  //fixed定点表示法
        cout << str_ts.str() << endl;
        // debug: 这里读取SAM+Matching的结果
        ifstream infile("./dataset/SePT/" + mSeq + "/mav0/semantic_bboxes_uf50/" + str_ts.str() + ".txt", ios::in);
        // 这里读取tracking第一帧初始化的结果
        // fstream infile("./dataset/SePT/" + mSeq + "/mav0/tracking_firstFrame_bbox/" + str_ts.str() + ".txt", ios::in);
        if (!infile.is_open())
        {
            cout << "semantic fusion fails" << endl;
            exit(1);
        }
        else
            cout << "read offline semantic boxes for semantic fusion" << endl;
        ...
        {
            BoxSE box;
            // box.m_class = mat_row[0]; // 默认class=1，类别是bicycle
            box.m_class = 66;
    ...
    // STEP 3. get current camera groundtruth by timestamp. 
    // string timestamp_string = to_string(timestamp);
    string timestamp_string = to_string(timestamp*1e9);
    // string timestamp_short_string = timestamp_string.substr(0, timestamp_string.length() - 4);
    string timestamp_short_string = timestamp_string.substr(0, timestamp_string.length() - 7);
    ...
    for (auto &row : mGroundtruth_mat)
    {
        ...
        string row_short_string = row_string.substr(0, row_string.length() - 7);

        if (row_short_string == timestamp_short_string)
        {
            // vector --> Eigen.
            truth_frame_poses(0) = row[0];  // timestamp
            truth_frame_poses(1) = row[1];  // euler_x
            truth_frame_poses(2) = row[2];  // euler_y
            truth_frame_poses(3) = row[3];  // euler_z
            truth_frame_poses(4) = row[5];  // quaternion_x
            truth_frame_poses(5) = row[6];  // quaternion_y
            truth_frame_poses(6) = row[7];  // quaternion_z
            truth_frame_poses(7) = row[4];  // quaternion_w
            ...
            // save to current frame.
            // cout << "tyl 553 get cam_pose_mat" << cam_pose_mat << endl;
            mCurrentFrame.mGroundtruthPose_mat = cam_pose_mat;
            // cout << "tyl 545 get groundtruth pose" << mCurrentFrame.mGroundtruthPose_mat  << endl;
                                     ...
                                     const bool bSemanticOnline)
{
    ...
    {
        // offline object box.
        ifstream infile("./dataset/SePT/" + mSeq + "/matching_bboxes_uf50/" + to_string(timestamp) + ".txt", ios::in);
        //ifstream infile("./data/yolo_txts/" + to_string(timestamp) + ".txt", ios::in);
        if (!infile.is_open())
        {
            cout << "bboxes file open fail" << endl;
            exit(1);
        }
        else
    ...
    string timestamp_string = to_string(timestamp);
    // std::cout << "timestamp_string" << timestamp_string << endl;
    string timestamp_short_string = timestamp_string.substr(0, timestamp_string.length() - 4);
    // std::cout << "timestamp_short_string" << timestamp_short_string << endl;
    ...
    for (auto &row : mGroundtruth_mat)
    {
        string row_string = to_string(row[0]); 
        string row_short_string = row_string.substr(0, row_string.length() - 4);
        // std::cout << "row_string" << row_string << endl;
        // std::cout << "row_short_string" << row_short_string << endl;
...
void Tracking::Track()
{
    ...
    if (mState == NOT_INITIALIZED)
    {
        if (mSensor == System::STEREO || mSensor == System::RGBD){
            ...
            cout << "stereoInitialzation finished. " << endl;
        }
    ...
    else
    {
        ...
        if (!mbOnlyTracking)
        {
            ...
            if (mState == OK)
            {
                ...
                else
                {
                    ...
                    // cout << "start TrackWithMotionModel." << endl;
                    bOK = TrackWithMotionModel();
                    // cout << "end TrackWithMotionModel." << endl;
                    if (!bOK)
                        bOK = TrackReferenceKeyFrame();
                }
            }
        ...
        else
        {
            ...
            else
            {
                if (!mbVO)
                {
                    ...
                    {
                        bOK = TrackWithMotionModel();
                        // cout << "end first TrackWithMotionModel()." << endl;
                    }
                    else
                    {
                        bOK = TrackReferenceKeyFrame();
                ...
                else
                {
                    ...
                    if (!mVelocity.empty())
                    {
                        ...
                        // cout << "end second TrackWithMotionModel()." << endl;
                    ...
                    if (bOKMM && !bOKReloc)
                    {
                        ...
                        if (mbVO)
                        {
                            cout << "retain the visual odometry solution." << endl;
                    ...
                    {
                        cout << "doing relocalization." << endl;
                        mbVO = false;
                    }
        ...
        cout << "start Track the local map." << endl;
        ...
        if (bOK)
        {
            ...
            {
                MapPoint *pMP = *lit;
                /*注释下行可以暂时保证某些轨迹完成全轨迹运行，但位姿估计结果可能会不准（物体错误位姿没有删除）*/
                delete pMP;    // mlpTemporalPoints指针的浅拷贝bug尚未修复，暂时不清除指向的指针
            }
        ...
        mLastFrame = Frame(mCurrentFrame);  // currentframe 和 lastframe 的关系->浅拷贝
    }
...
void Tracking::StereoInitialization()
{
    ...
    {   
        ...
        for (int i = 0; i < mCurrentFrame.N; i++)
        {
            // cout << "1103 tyl mcurrent.mappoint: " << mCurrentFrame.mvpMapPoints[i] << endl;
            ...
            // cout << "1117 tyl mcurrent: " << mCurrentFrame.mvpMapPoints[i] << endl;
        }
        // 暂时没有起到作用
        // [PO-SLAM] modify: rotate the world coordinate to the current frame. //2023.11.18
        ...
        bool build_worldframe_on_ground = true;  // 使得重力方向垂直于世界坐标系下的地面
...
void Tracking::MonocularInitialization()
{
    ...
    {
        cout << "Keypoints: " << mCurrentFrame.mvKeys.size() << endl;

        // Try to initialize
        if ((int)mCurrentFrame.mvKeys.size() <= 100)   // keypoints are sufficient
        {
            delete mpInitializer;
        ...
        cout << "Check if there are enough correspondences: " << nmatches << endl;
        ...
        if (mpInitializer->Initialize(mCurrentFrame, mvIniMatches, Rcw, tcw, mvIniP3D, vbTriangulated))
        {
            ...
            cout <<  "finish the second frame initialization..." << endl;
...
void Tracking::UpdateLastFrame()
{
    ...
    for (size_t j = 0; j < vDepthIdx.size(); j++)
    {
        ...
        if (bCreateNew)
        {
            ...
            // MapPoint *pNewMP2 = new MapPoint(x3D, mpMap, &mLastFrame, i);  //2023.11.18 tyl - deepcopy test

            mLastFrame.mvpMapPoints[i] = pNewMP;  // 浅拷贝

            mlpTemporalPoints.push_back(pNewMP);  // 浅拷贝 //2023.11.18 tyl - deepcopy test

            // cout << "tyl 1479 to test the relationship between mlpTemporalPoints and mLastFrame" << endl;
            // cout << "tyl 1479 the adress for pNewMP:" << pNewMP << endl;
            // cout << "tyl 1479 the adress for mLastFrame.mvpMapPoints:" << mLastFrame.mvpMapPoints[i] << endl;
            // MapPoint* currentPoint= mlpTemporalPoints.back();
            // cout << "tyl 1479 the adress for mlpTemporalPoints:" << currentPoint << endl;
            nPoints++;
        }
        else
        {
...
bool Tracking::TrackWithMotionModel()
{
    ...
    if((mCurrentFrame.mnId > 10) && bCubeslam)
    {
        ...
        // TUM3
        // calib << 535.4,  0,  320.1,   // fx, 0 ,cx
        //          0,  539.2, 247.6,    // 0, fy, cy
        //          0,      0,     1;    // 0, 0, 1
        // 2023.11.16手动修改calib
        // SePT
        calib << 610.17, 0,  512.0,
                 0,  610.17, 512.0,
                 0,      0,     1;
    ...
    // cout << "step8 finished." << endl;

    // ************************************
    // STEP 9. Initialize the object map  *
    // ************************************
    // 双目没有mInitialSecendFrame，物体地图的初始化可能失败，因此做如下改写：
    if ((mCurrentFrame.mnId > mInitialSecendFrame.mnId) && mbObjectIni == false && mSensor==System::MONOCULAR)
        InitObjMap(objs_2d);
    if ((mCurrentFrame.mnId > mInitialFrame.mnId) && mbObjectIni == false && (mSensor==System::STEREO||mSensor==System::RGBD))
        InitObjMap(objs_2d);

    // cout << "step9 finished." << endl;
    ...
    if ((mCurrentFrame.mnId > mnObjectIniFrameID) && (mbObjectIni == true))
    {
        ...
        for (int i = 0; i < (int)mpMap->mvObjectMap.size(); i++)
        {
            ...
            // tyl: 限制物体被观测的帧数——目前是最后30帧
            // 轨迹较短的时候，只有最后30帧可能无法充分被观测，目前扩充至最后50帧

            // if (mpMap->mvObjectMap[i]->mnLastAddID > mCurrentFrame.mnId - 30){
            //     mpMap->mvObjectMap[i]->ComputeProjectRectFrame(image, mCurrentFrame);
            // }
            if (mpMap->mvObjectMap[i]->mnLastAddID > mCurrentFrame.mnId - 50){
                // cout << "enter ComputeProjectRectFrame in step10" << endl;                         //tyl
                // int tmpsize =  mpMap->mvObjectMap[i]->mvpMapObjectMappoints.size();                //zjh - 2023.11.16
                // cout << mpMap->mvObjectMap[i]->mvpMapObjectMappoints[tmpsize-1]->GetWorldPos();    //zjh - 2023.11.16
                mpMap->mvObjectMap[i]->ComputeProjectRectFrame(image, mCurrentFrame);
                // cout << "enter ComputeProjectRectFrame in step10" << endl;
            }
            else
            {
