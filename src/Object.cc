...
void Object_2D::ObjectDataAssociation(Map *mpMap, Frame &mCurrentFrame, cv::Mat &image, string &flag)
{
    ...
    if (old == false)
    {
        ...
        cout <<"zjh 682 Obj_c_MapPonits.size()" <<Obj_c_MapPonits.size() <<endl;
        ...
        for (size_t i = 0; i < Obj_c_MapPonits.size(); i++)
        {
            ...
            cout<<"zjh 689 vpMapObjectMappoints [ " <<i<<"] "<< pMP->object_id << endl;                          
...
void Object_Map::ComputeMeanAndStandard()
{
    ...
    // remove bad points.
    {
        ...
        vector<MapPoint *>::iterator pMP;
        int i = 0;
        cout << "erase 979 tyl mvpMapObjectMappoints.size:" << mvpMapObjectMappoints.size() << endl;
             ...
             pMP != mvpMapObjectMappoints.end();)
        {
            ...
            if ((*pMP)->isBad())
            {
                cout << "PointPosWorld to be erased 979: " << pos << endl;
        ...
        cout << "erase after 979 tyl mvpMapObjectMappoints.size:" << mvpMapObjectMappoints.size() << endl;
    }
...
void Object_Map::IsolationForestDeleteOutliers()
{
    ...
    int numMOutpoint = 0;
    {
        ...
        vector<MapPoint *>::iterator pMP;
        cout << "erase 1297 tyl mvpMapObjectMappoints.size:" << mvpMapObjectMappoints.size() << endl;
        for (pMP = mvpMapObjectMappoints.begin();
             pMP != mvpMapObjectMappoints.end();)
        {
            numMappoint++;
            ...
            {
                numMOutpoint++;
                cout << "PointPosWorld to be erased 1297: " << pos << endl;
                pMP = mvpMapObjectMappoints.erase(pMP); 
                mSumPointsPos -= pos;
            }
            else
        ...
        cout << "erase after 1297 tyl mvpMapObjectMappoints.size:" << mvpMapObjectMappoints.size() << endl;
    }
                                     ...
                                     int Flag) // 1 Iou, 2 NP, 3 t-test，4 project. for debug.
{
    ...
    if ((Flag != 1) && (Flag != 4))
    {
        ...
        cout << "projected bounding box2.1397"<<endl;
    ...
    for (size_t j = 0; j < ObjectFrame->Obj_c_MapPonits.size(); ++j)
    {
...

        {
            ...
            {
                cout<<" zjh 1493"<<endl;
                cout <<"zjh 1493 Obj_c_MapPonits.size()" <<pMP->GetWorldPos() <<endl;
        ...
        (ObjectFrame->mBox.y + ObjectFrame->mBox.height < image.rows - 25))
    {
        unique_lock<mutex> lock(mMutexMapPoints); // lock.
        vector<MapPoint *>::iterator pMP;
        cout << "erase 1514 tyl mvpMapObjectMappoints.size:" << mvpMapObjectMappoints.size() << endl;
        for (pMP = mvpMapObjectMappoints.begin();
             pMP != mvpMapObjectMappoints.end();)
        {
            int sit_sec = 0;
            ...
            if ((u > 0 && u < image.cols) && (v > 0 && v < image.rows))
            {
                if (!ObjectFrame->mBoxRect.contains(cv::Point2f(u, v)))
                {
                    ...
                    cout << "PointPosWorld to be erased 1514: " << PointPosWorld << endl;
                    pMP = mvpMapObjectMappoints.erase(pMP);
        ...
        cout << "erase 1560 tyl mvpMapObjectMappoints.size:" << mvpMapObjectMappoints.size() << endl;
    }
...
void Object_Map::ComputeProjectRectFrame(cv::Mat &image, Frame &mCurrentFrame)    //zjh
{
    const cv::Mat Rcw = mCurrentFrame.mTcw.rowRange(0, 3).colRange(0, 3);
    const cv::Mat tcw = mCurrentFrame.mTcw.rowRange(0, 3).col(3);
    ...
    vector<float> x_pt;
    vector<float> y_pt;
    // cout << "current num: " <<  "Rcw.type: " << Rcw.type()  << endl;           //zjh
    // cout << "rotation: " << Rcw << endl;                                       //zjh
    // cout << "PointPos size: " << mvpMapObjectMappoints.size() << endl;         //zjh
    // vector< MapPoint*> MymvpMapObjectMappoints = mvpMapObjectMappoints;        //zjh
    for (int j = 0; j < mvpMapObjectMappoints.size(); j++)
    {
        cout << "for  start "<< j << endl;                                     //zjh
        MapPoint *pMP = mvpMapObjectMappoints[j];
        // cout << "pMP get" << endl;                                                //zjh
        cout << "PointPosWorld get" << pMP->GetWorldPos() << endl;                //zjh

        cv::Mat PointPosWorld = pMP->GetWorldPos();

        if(Rcw.type()!= PointPosWorld.type()){                                   // 2023.11.15
            if(PointPosWorld.type() == 0){ // 如果PointPosWorld为空                // 2023.11.15
                cout << "PointPosWorld为空" << endl;
                // 如果上一个点的位姿存在，则使用上一个点的类型作为目标类型
                if(mvpMapObjectMappoints[j-1]->have_feature){
                    PointPosWorld = mvpMapObjectMappoints[j-1]->GetWorldPos();
                }
                // break;                                                        // 2023.11.16 zjh
                else{
                    PointPosWorld = cv::Mat::ones(3,1, Rcw.type());              // 2023.11.16, 随机赋予一个初始数值 
                }
            }// 使用 Rcw 的数据类型作为目标类型 
        }
...
void Object_Map::MergeTwoMapObjs(Object_Map *RepeatObj)
{
    ...
    for (int i = 0; i < RepeatObj->mvpMapObjectMappoints.size(); i++)
    {
        ...
        }
        {
            ...
            {
                cout<<"zjh 1816"<< endl;
                cout<<"zjh 1816"<<pMP->GetWorldPos()<< endl;
                mvpMapObjectMappoints.push_back(pMP);
...
void Object_Map::BigToSmall(Object_Map *SmallObj, float overlap_x, float overlap_y, float overlap_z)
{
    ...
    //  0------1
    {
        ...
        vector<MapPoint *>::iterator pMP;
        cout << "erase 2064 tyl mvpMapObjectMappoints.size:" << mvpMapObjectMappoints.size() << endl;
        for (pMP = mvpMapObjectMappoints.begin();
             pMP != mvpMapObjectMappoints.end();)
        {
            cv::Mat PointPosWorld = (*pMP)->GetWorldPos();
            ...
            if ((PointPosWorld.at<float>(0) > SmallObj->mCuboid3D.x_min) && (PointPosWorld.at<float>(0) < SmallObj->mCuboid3D.x_max) &&
                ...
                cout << "PointPosWorld to be erased 2064: " << PointPosWorld << endl;
                pMP = mvpMapObjectMappoints.erase(pMP);
                }
            else
            {
        ...
        cout << "erase after 2064 tyl mvpMapObjectMappoints.size:" << mvpMapObjectMappoints.size() << endl;
    }
...
{
    {
        ...
        cout<<"erase 2094 tyl mvpMapObjectMappoints.size:"<< mvpMapObjectMappoints.size() << endl;
        for (pMP = mvpMapObjectMappoints.begin();
             pMP != mvpMapObjectMappoints.end();)
        {
            cv::Mat PointPosWorld = (*pMP)->GetWorldPos();
            ...
            if (((PointPosWorld.at<float>(0) > AnotherObj->mCuboid3D.cuboidCenter(0) - (AnotherObj->mCuboid3D.lenth / 2 - overlap_x / 2)) &&
                ...
                ((PointPosWorld.at<float>(2) > AnotherObj->mCuboid3D.cuboidCenter(2) - (AnotherObj->mCuboid3D.height / 2 - overlap_z / 2)) &&
                 (PointPosWorld.at<float>(2) < AnotherObj->mCuboid3D.cuboidCenter(2) + (AnotherObj->mCuboid3D.height / 2 - overlap_z / 2))))
            {
                cout << "PointPosWorld to be erased 2094: " << PointPosWorld << endl;
                pMP = mvpMapObjectMappoints.erase(pMP);
            }
        ...
        cout<<"erase after 2094 tyl mvpMapObjectMappoints.size:"<< mvpMapObjectMappoints.size() << endl;
    }
